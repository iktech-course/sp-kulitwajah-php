...
$stmt->prepare($query);
$stmt->bind_param('si', $user['username'], $user['user_id']);
$stmt->execute();
...
if($row != null){
    ...
    //jika password dirubah
    if($_POST['password'] !== ''){
	    $stmt->bind_param('sssi', $user['email'],$user['username'],$password, $user['user_id']);
    }else{
	    $stmt->bind_param('ssi', $user['email'],$user['username'], $user['user_id']);
    }
	$result = $stmt->execute();
